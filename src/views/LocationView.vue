...
<template>
    ...
    <div class="serach" v-else>
      <div class="search-input">
        ...
        <!-- <button  @click="weatherData">
        </button> -->
        <input type="text" v-model="cityName" placeholder="Поиск локации" @keyup.enter="weatherData">
      ...
      <p class="center" v-if="city == null && saved.length == 0">У вас пока нет избранных локаций</p>
      <div class="saved" >
        <LocationItem v-for="item in saved" :key="item" 
        :weather="item.weather" :currentTemp="item.currentTemp" :city="item.city"
        />
      </div>
      <div class="saved" v-if="!isSaved && city != null">
        <LocationItem :weather="weather" :currentTemp="currentTemp" :city="city"/>
      </div>
    </div>
</template>
...
<script>
  ...
  export default {
  components: { LocationItem, PreLoaderVue},
    data() {
      return {
        cityName: '',
        error: null,
